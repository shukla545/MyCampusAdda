...
const marketplaceBaseValidation = ({ titleMin = 4, branchRequired = true } = {}) => [
  body('title').trim().isLength({ min: titleMin }).withMessage('Product title is required'),
...
const marketplaceUpdateValidation = [
  ...marketplaceBaseValidation({ titleMin: 2, branchRequired: false }),
  body('description').trim().isLength({ min: 3 }).withMessage('Write at least 3 characters about the product'),
...
router.put('/:id', protectUser, marketplaceUpdateValidation, validate, updateMyMarketplaceListing);
